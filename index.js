...
async function buildImportList(startingFiles) {
  ...
  async function processFile(file) {
    ...
    await Promise.all(root.find(j.CallExpression)
      ...
      .map(async node => {
        ...
        if (args.length !== 1 || args[0].type !== 'StringLiteral') {
          throw new Error(`unsupported complex require-glob in ${file} (${node.loc.start.line}:${node.loc.start.column}):
    ${recast.print(node).code}`)
        }
        const files = await promisify(glob)(path.resolve(path.dirname(file), args[0].value))
        await Promise.all(files.map(moduleName => processImport(file, moduleName)))
      })
    )
    ...
    await Promise.all(root.find(j.CallExpression).filter(path =>
      ...
      if (arg.type !== 'StringLiteral') {
        throw new Error(`unsupported dynamic path in ${file} (${node.loc.start.line}:${node.loc.start.column}):
  ${recast.print(node).code}`)
      }
...
function findRequireGlobBinding(root) {
  ...
  const requireGlobImport = root.find(j.ImportDeclaration, {source: {value: 'require-glob'}})
  if (requireGlobImport.size()) requireGlobImport.find(j.ImportDefaultSpecifier).forEach(path => {
    requireGlobBinding = path.node.local
  })
  if (requireGlobBinding) return requireGlobBinding
  const requireGlobDeclarator = root.find(j.CallExpression, {
    callee: {name: 'require'},
    arguments: [{type: 'StringLiteral', value: 'require-glob'}],
  }).filter(isTrueRequire).closest(j.VariableDeclarator)
  ...
  return _path => {
    ...
    if (requireGlobBinding) {
      if (callee.type === 'MemberExpression' &&
        ...
        if (!bindings || !bindings.length || bindings[0].node !== requireGlobBinding) return false
